...
//    dispatcher = std::make_shared<btCollisionDispatcher>( collisionConfig.get() );
//    broadphase = std::make_shared<btDbvtBroadphase>();
//    solver = std::make_shared<btSequentialImpulseConstraintSolver>();
//    world = std::make_shared<btDiscreteDynamicsWorld>( dispatcher.get(), broadphase.get(), solver.get(), collisionConfig.get() );
    world->setGravity( btVector3( 0, -10, 0 ) );

    btTransform t;
    t.setIdentity();
    t.setOrigin( btVector3( 0, -0.4f, 0 ) );
    btStaticPlaneShape *plane = new btStaticPlaneShape( btVector3( 0, 1, 0 ), 0 );
    btMotionState *motion = new btDefaultMotionState( t );
//    auto plane = std::make_shared<btStaticPlaneShape>( btVector3( 0, 1, 0 ), 0 );
//    auto motion = std::make_shared<btDefaultMotionState>( t );
...
void Scene::slotUpdate()
{
    ...
    m_xTra = m_R * cos( ( double ) gltDegToRad( m_angle ) );
    m_zTra = m_R * sin( ( double ) gltDegToRad( m_angle ) );
    ...
    world->stepSimulation( 1.0f/60.f );

    updateGL();
}

btRigidBody *Scene::addSphere( float rad, float x, float y, float z, float mass )
{
    btTransform t;
    t.setIdentity();
...
btRigidBody *Scene::addBox( float x, float y, float z, float mass )
{
    btTransform t;
    t.setIdentity();
    t.setOrigin( btVector3( x, y, z ) );
...
void Scene::renderSphere( btRigidBody *sphere )
{
    if ( sphere->getCollisionShape()->getShapeType() != SPHERE_SHAPE_PROXYTYPE ) {
        return;
    }
    //glColor3f( 1, 0, 0 );
    float r = static_cast<btSphereShape*>( sphere->getCollisionShape() )->getRadius();
    btTransform t;
...
void Scene::renderPlane( btRigidBody *plane )
{
...
void Scene::renderBox( btRigidBody *box )
{
    if ( box->getCollisionShape()->getShapeType() != BOX_SHAPE_PROXYTYPE ) {
        return;
    }
    //glColor3f( 1, 0, 0 );
    //static_cast<btBoxShape*>( sphere->getCollisionShape() )->
    btTransform t;
...
//std::shared_ptr<btRigidBody> Scene::addSphere( float rad, float x, float y, float z, float mass )
//{
//    btTransform t;
//    t.setIdentity();
//    t.setOrigin( btVector3( x, y, z ) );
...
//void Scene::renderSphere(std::shared_ptr<btRigidBody> sphere)
//{
//    if ( sphere->getCollisionShape()->getShapeType() != SPHERE_SHAPE_PROXYTYPE ) {
//        return;
//    }
...
//void Scene::renderPlane(std::shared_ptr<btRigidBody> plane)
//{
//    if ( plane->getCollisionShape()->getShapeType() != SPHERE_SHAPE_PROXYTYPE ) {
//        return;
//    }
//    glColor3f( 0.8f, 0.8f, 0.8f );
//    btTransform t;
...
void Scene::keyPressEvent( QKeyEvent *event )
{
    switch ( event->key() ) {
        ...
        case Qt::Key_Right:
            ...
            break;
        case Qt::Key_Space:
//            btRigidBody *sphere = addSphere( 1.0f, 0.0f, 2.0f, 5.0f, 1.0f );
//            sphere->setLinearVelocity( btVector3( 0.0f, 2.0f, 5.0f ) );
            btRigidBody *box = addBox( 0.0f, 5.0f, 5.0f, 5.0f );
            box->setLinearVelocity( btVector3( 5.0f, 5.0f, 5.0f ) );
...
void Scene::genTexture()
{
    ...
    m_cubeTextureID=bindTexture(QPixmap(QString(":textures/picture1.jpg")), GL_TEXTURE_2D);
    glTexParameteri(GL_TEXTURE_2D, GL_TEXTURE_WRAP_S, GL_CLAMP_TO_EDGE);
    glTexParameteri(GL_TEXTURE_2D, GL_TEXTURE_WRAP_T, GL_CLAMP_TO_EDGE);
